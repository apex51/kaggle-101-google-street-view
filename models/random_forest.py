...
def read_image(path, imageIndex, imageSize):
    ...
    for index in imageIndex:
        ...
        fd = hog(image, orientations=8, pixels_per_cell=(5, 5), cells_per_block=(1, 1))
        dataSet.append(fd)
    # Turn dataSet into a 0/1 matrix.  
    return mat(dataSet)
...
testData = read_image('C:/Users/Victor/Desktop/M/test40', testIndex, imageSize)
trainFeat = [str_to_feature(trainLabel[index]) for index in trainIndex]

# Train the RF model.
model = RandomForestClassifier(n_estimators=400, n_jobs=-1)
scores = cross_validation.cross_val_score( model, trainData, trainFeat, cv=5)
print scores.mean(), scores.std() ** 2

# model.fit(trainData, trainFeat)
# print model.score(trainData, trainFeat)
